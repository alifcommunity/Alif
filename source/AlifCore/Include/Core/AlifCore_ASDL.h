...
#define GENERATE_SEQ_CONSTRUCTOR(typeName, name, type) ASDL ## typeName ## Seq* alifNew_ ## name ## Seq(AlifSizeT _size, AlifASTMem* _astMem) { \
	ASDL ## typeName ## Seq* seq_ = nullptr; \
	AlifUSizeT n_{}; \
	...
	if (n_ > SIZE_MAX - sizeof(ASDL ## typeName ## Seq)) { \
		/* error */ \
		return nullptr; \
	} \
	n_ += sizeof(ASDL ## typeName ## Seq); \
	seq_ = (ASDL ## typeName ## Seq*)alifASTMem_malloc(_astMem, n_); \
	if (!seq_) { \
		/* error */ \
		return nullptr; \
	} \
